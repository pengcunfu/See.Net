...
- 托盘常驻：启动后只在系统托盘运行，不打开文件浏览器主窗口。
- 空格预览：在资源管理器中选中文件后按空格，弹出 Quick Look 式预览浮窗；再按空格或 Esc 关闭，↑/↓ 切换多选文件。
- 打开文件：托盘菜单「打开文件预览…」、或用命令行/关联打开文件，直接弹出预览浮窗。
